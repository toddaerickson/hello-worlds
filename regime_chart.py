"""Generate a static PNG chart of the historical regime score (1980-2026).

Produces a multi-panel chart:
  - Top panel: Composite regime score with S&P 500 price overlay, event
    annotations, fiscal dominance shading, and zone bands.
  - Bottom panels: 7 individual signal scores + FD conditions met.

Usage:
    python regime_chart.py          # outputs regime_chart.png
"""
...
def generate_png(output_path="regime_chart.png"):
    """Build and save the regime dashboard chart as a PNG file.
    ...
    # Signal sub-chart definitions: (data key, display label, line color)
    signal_keys = [
    ...
    # Key market events: (date, label, color)
    # Red = crashes/crises, orange = stress events, green = bottoms, purple = structural
    events = [
        ("1981-06", "Volcker\nPeak", "#e67e22"),
        ("1987-10", "Black\nMonday", "#e74c3c"),
        ("1990-07", "Gulf War\nRecession", "#e74c3c"),
    ...
    # =====================================================================
    # Main chart (top half of the figure)
    # =====================================================================
    ...
    # S&P 500 price overlay on secondary y-axis (log scale)
    # Log scale because SPX goes from ~114 (1980) to ~5700 (2026)
    ...
    # X-axis: show year labels every 4 years (46-year range needs wide spacing)
    tick_positions = []
    tick_labels = []
    ...
    # =====================================================================
    # Signal sub-charts (bottom half: 4 columns x 2 rows = 8 panels)
    # 7 signals + 1 FD conditions panel
    # =====================================================================
    cols = 4
    sub_w = 0.88 / cols
    sub_h = 0.18
    x_start = 0.06
    y_starts = [0.27, 0.05]  # Top row and bottom row y-positions

    for i, (key, label, color) in enumerate(signal_keys):
        row = i // cols
        col = i % cols
        ax = fig.add_axes([x_start + col * sub_w + 0.01, y_starts[row],
                           sub_w - 0.02, sub_h])
        ax.set_facecolor("#111827")

        # Plot the signal's 0-100 score over time
        ...
        # Sparse x ticks (every 4 years) to avoid clutter
        sparse_ticks = [j for j, d in enumerate(dates)
                        if d.endswith("-01") and int(d[:4]) % 4 == 0]
    ...
    # --- FD conditions panel (8th slot) ---
    # Bar chart showing how many of the 4 FD conditions are met each month
    fd_conditions = [d["fd_conditions"] for d in data]
    row = len(signal_keys) // cols
    col = len(signal_keys) % cols
    ax_fd = fig.add_axes([x_start + col * sub_w + 0.01, y_starts[row],
                          sub_w - 0.02, sub_h])
    ax_fd.set_facecolor("#111827")

    # Purple bars when >= 3 conditions met (flag active), gray otherwise
